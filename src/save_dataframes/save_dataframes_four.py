...
import gc
#%%
cols_to_keep_reviews = ['reviewerID', 'asin', 'overall']
cols_to_keep_meta = ['asin', 'description', 'category'] 

food_data = pd.read_json('../dataset/reviews_Grocery_and_Gourmet_Food_5.json', lines=True)[cols_to_keep_reviews]
food_meta = pd.read_json('../dataset/meta_Grocery_and_Gourmet_Food.json', lines=True)[cols_to_keep_meta]

merged_food = pd.merge(food_data, food_meta, on='asin', how='left')
food = merged_food.rename(columns={'reviewerID': 'uid', 'asin': 'iid', 'overall': 'y'})

del merged_food, food_data, food_meta
gc.collect()
...
for ratio in ratio_information:
    ...
    save(train_src, train_tgt, train_meta, test, ratio, "movie", "food")



#%%
# rm -r tgt_food_src_kitchen/
# mkdir tgt_food_src_movie
