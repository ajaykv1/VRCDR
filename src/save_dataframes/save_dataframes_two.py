...
#%%
games_data = pd.read_json('../dataset/reviews_Video_Games_5.json', lines=True)
games_meta = pd.read_json('../dataset/meta_Video_Games.json', lines=True)

merged_games = pd.merge(games_data, games_meta, on='asin', how='left')

columns_to_keep = ['reviewerID', 'asin', 'overall', 'category', 'description']
games = merged_games[columns_to_keep]
games = games.rename(columns={'reviewerID': 'uid', 'asin': 'iid', 'overall': 'y'})
#%%
def mapper(src, tgt):
...
for ratio in ratio_information:
    ...
    save(train_src, train_tgt, train_meta, test, ratio, "electronics", "games")
