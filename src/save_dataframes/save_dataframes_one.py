...
#%%
music_data = pd.read_json('../dataset/reviews_CDs_and_Vinyl_5.json', lines=True)
music_meta = pd.read_json('../dataset/meta_CDs_and_Vinyl.json', lines=True)

merged_music = pd.merge(music_data, music_meta, on='asin', how='left')

columns_to_keep = ['reviewerID', 'asin', 'overall', 'category', 'description']
music = merged_music[columns_to_keep]
music = music.rename(columns={'reviewerID': 'uid', 'asin': 'iid', 'overall': 'y'})
#%%
def mapper(src, tgt):
...
for ratio in ratio_information:
    ...
    save(train_src, train_tgt, train_meta, test, ratio, "movie", "music")
