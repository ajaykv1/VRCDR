...
#%%
tgt_data = pd.read_csv(f'../project_data/{args.ratio}/tgt_{args.tgt}_src_{args.src}/train_tgt.csv', names=['uid', 'iid', 'y', 'category', 'description'])
print(tgt_data)
...
unique_items = tgt_data.drop_duplicates(subset='iid')
...
description_embeddings = np.array(tgt_data['iid'].map(description_embeddings_dict).tolist())
category_embeddings = np.array(tgt_data['iid'].map(category_embeddings_dict).tolist())
#%%
print(tgt_data)
print(tgt_data.columns)
print(len(tgt_data))
#%%
n_users = max(tgt_data['uid'])
n_items = max(tgt_data['iid'])
...
#%%
# Add them to X
X = [tgt_data['uid'].values, tgt_data['iid'].values, category_embeddings, description_embeddings]
...
for lr in learning_rates:
    ...
    for train, val in kfold.split(X[0], y):
        ...
        checkpoint = ModelCheckpoint(f'../project_data/{args.ratio}/tgt_{args.tgt}_src_{args.src}/best_models/pre_tgt_best_model_lr_{lr}_fold_{fold_no}.h5', monitor='val_loss', save_best_only=True)
        ...
        # Load the best model
        model.load_weights(f'../project_data/{args.ratio}/tgt_{args.tgt}_src_{args.src}/best_models/pre_tgt_best_model_lr_{lr}_fold_{fold_no}.h5')
...
best_model.save(f'../project_data/{args.ratio}/tgt_{args.tgt}_src_{args.src}/tgt_model.h5')
#%%
end_time = time.time()
